CurrentVsTime <- function(CyclingData,
  ...
  RequiredCols <- c("File", "Cycle", "Current", "TestTime", "StepType")
  ...
  PlotData <- CyclingData |>
    filter(!is.na(Cycle), !is.na(Current), !is.na(TestTime)) |>
    group_by(File) |>
  ...
  walk(Files, function(Fname) {
    ...
    CurrentRange <- range(FileData$Current,    na.rm = TRUE)
    TimePad   <- diff(TimeRange) * PaddingPct
    CurrentPad   <- diff(CurrentRange) * PaddingPct

    XLim <- c(TimeRange[1] - TimePad, TimeRange[2] + TimePad)
    YLim <- c(CurrentRange[1] - CurrentPad, CurrentRange[2] + CurrentPad)
